...
PONT removerUmaOcorrencia(PONT raiz, int valor) {
    ...
    if(raiz == NULL)
        return raiz;
    if(raiz->chave > valor)
        removerUmaOcorrencia(raiz->esq, valor);
    if(raiz->chave < valor)
        removerUmaOcorrencia(raiz->dir, valor);
    if (raiz->chave == valor)
    {
        raiz->contador -= 1;
        if(raiz->contador == 0)
        {
            ...
            PONT sucessor = pegarSucessor(raiz);
            raiz->chave = sucessor->chave;
            raiz->contador = sucessor->contador;
            raiz->dir = removerTodasOcorrencias(raiz->dir, sucessor->chave);
        }
    }
...
PONT removerTodasOcorrencias(PONT raiz, int valor) {
    if(raiz == NULL)
        return raiz;
    if(raiz->chave > valor)
        removerTodasOcorrencias(raiz->esq, valor);
    if(raiz->chave < valor)
        removerTodasOcorrencias(raiz->dir, valor);
    if (raiz->chave == valor)
    {
...
int contarTotalElementos(PONT raiz) {
    ...
    if(raiz == NULL)
        return 0;
    contarTotalElementos(raiz->esq);
    elementos += raiz->contador;
    if(!(raiz->esq != NULL)!= !(raiz->dir != NULL))
        elementos += 1;
    if(raiz->esq != NULL && raiz->dir != NULL)
        elementos += 2;
    contarTotalElementos(raiz->dir);
    // COMPLETAR
    // soma = raiz->contador + subárvores
    return elementos; // provisório
}

//------------------------------------------------------------------------------
// 10) k-ésimo menor
int kEsimoMenorAjuda(PONT raiz, int menor, int k) {
    if(raiz == NULL)
        return -1;
    
    int esq = kEsimoMenorAjuda(raiz->esq, menor, k);
    if (esq != -1) return esq;
    menor++;
    if(menor == k) return raiz->chave;

    int dir = kEsimoMenorAjuda(raiz->dir, menor, k);
    return dir;
}
...
int kEsimoMenor(PONT raiz, int k) {
    // COMPLETAR
    int menor = 0;
    return kEsimoMenorAjuda(raiz, menor, k); // provisório
}
